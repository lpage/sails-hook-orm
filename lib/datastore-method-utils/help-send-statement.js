...
module.exports = function helpSendStatement(options, done){
  ...
  queryType = (function setQueryType() {
    var QUERY_TYPES = ['avg', 'sum', 'count', 'del', 'update', 'insert', 'select'];
    var len = QUERY_TYPES.length;
    var qType;
    while (len--) {
      qType = QUERY_TYPES[len];
      if (hasOwnProp.call(options.statement, qType)) {
        //account for "delete" pattern inconsistency
        return qType === 'del' ? 'delete' : qType;
      }
    }
    return 'NOT_FOUND';
  }());

  if (queryType === 'NOT_FOUND') {
